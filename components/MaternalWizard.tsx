...
interface MaternalWizardProps {
  isOpen: boolean;
  onComplete: (data: MaternalWizardData) => void;
  onSkip: () => void;
  motherName: string;
}

export interface MaternalWizardData {
  maternalGrandmother: string;
  maternalGrandfather: string;
  grandparentsNotTogether: boolean;
  motherSiblings: Array<{
    name: string;
    gender: 'male' | 'female';
    isMarried: boolean;
    spouseName?: string;
...
export const MaternalWizard: React.FC<MaternalWizardProps> = ({
  isOpen,
  onComplete,
  onSkip,
  motherName
  ...
  // Step 2: Mother's siblings count
  const [siblingCount, setSiblingCount] = useState<number>(0);

  // Step 3: Sibling details
  const [motherSiblings, setMotherSiblings] = useState<Array<{
    name: string;
    gender: 'male' | 'female';
    isMarried: boolean;
    spouseName?: string;
  ...
  const updateSiblingCount = (count: number) => {
    ...
    setMotherSiblings(Array(count).fill(null).map(() => ({
      name: '',
      gender: 'male',
      isMarried: false,
      spouseName: '',
  ...
  const handleComplete = () => {
    const data: MaternalWizardData = {
      maternalGrandmother: maternalGrandmother.trim(),
      maternalGrandfather: maternalGrandfather.trim(),
  ...
  const allSiblingsValid = motherSiblings.every(s => s.name.trim() && (!s.isMarried || s.spouseName?.trim()));
      ...
      <DialogContent className="sm:max-w-[500px]" aria-describedby={undefined}>
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Add Maternal Family (Mom's Side)
          </DialogTitle>
          <DialogDescription>
            {step === 1 && `Tell us about ${motherName}'s parents (your maternal grandparents)`}
            {step === 2 && `How many siblings does ${motherName} have?`}
            {step === 3 && `Tell us about ${motherName}'s siblings (your aunts & uncles)`}
          </DialogDescription>
        </DialogHeader>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="maternal-grandmother">👵 Maternal Grandmother's Name</Label>
              <Input
                id="maternal-grandmother"
                value={maternalGrandmother}
                onChange={(e) => setMaternalGrandmother(e.target.value)}
                placeholder={`e.g., ${motherName}'s mother`}
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor="maternal-grandfather">👴 Maternal Grandfather's Name</Label>
              <Input
                id="maternal-grandfather"
                value={maternalGrandfather}
                onChange={(e) => setMaternalGrandfather(e.target.value)}
                placeholder={`e.g., ${motherName}'s father`}
              />
            </div>

            {maternalGrandmother.trim() && maternalGrandfather.trim() && (
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>How many siblings does {motherName} have? (Your aunts/uncles)</Label>
